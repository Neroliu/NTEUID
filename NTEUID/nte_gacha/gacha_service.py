...
async def run_bind_tap(bot: Bot, ev: Event, tap_id_arg: str) -> None:
    ...
    if user.tap_id and user.tap_id != tap_id_str:
        return await send_nte_notify(bot, ev, GachaMsg.BIND_REPLACE_WARNING)
    ...
    logger.info(f"[NTE抽卡] 绑定成功 user_id={ev.user_id} center_uid={user.center_uid} tap_id={tap_id}")
    await send_nte_notify(bot, ev, GachaMsg.bind_success(binding.name))


async def run_my_gacha(bot: Bot, ev: Event, tap_id_arg: str = "") -> None:
    """`抽卡记录 [<tap_id>]`：
    - 不带参数：走绑定的 tap_id（要求已登录 + 已绑定）
    - 带参数 + NTEGachaUnsafeQuery=ON：直查任意 tap_id
    - 带参数 + NTEGachaUnsafeQuery=OFF：忽略参数，回退绑定路径
    """
    arg = tap_id_arg.strip()
    if arg and bool(NTEConfig.get_config("NTEGachaUnsafeQuery").data):
        tap_id = _normalize_tap_id(arg)
        if tap_id is None:
            return await send_nte_notify(bot, ev, GachaMsg.INVALID_TAP_ID)
        return await _send_summary(bot, ev, tap_id=tap_id, fallback_role_name="")
    ...
    await _send_summary(bot, ev, tap_id=tap_id, fallback_role_name=user.role_name)


async def _send_summary(bot: Bot, ev: Event, *, tap_id: int, fallback_role_name: str) -> None:
    try:
        binding = await taptap.check_binding(tap_id)
    except SdkError as err:
        logger.warning(f"[NTE抽卡] 查询失败 user_id={ev.user_id} tap_id={tap_id} err={err.message}")
        return await send_nte_notify(bot, ev, GachaMsg.LOAD_FAILED)
    if not binding.is_bind:
        return await send_nte_notify(bot, ev, GachaMsg.TAPTAP_NOT_BOUND)
    ...
    try:
        summary = await taptap.gacha_summary(tap_id)
    except SdkError as err:
        logger.warning(f"[NTE抽卡] 抽卡总览失败 user_id={ev.user_id} tap_id={tap_id} err={err.message}")
        return await send_nte_notify(bot, ev, GachaMsg.LOAD_FAILED)
    if summary.is_empty:
