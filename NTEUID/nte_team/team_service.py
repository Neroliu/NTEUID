...
def _filter_recommendations(
    ...
    for recommendation in recommendations:
        recommendation_name = alias_to_role_name(recommendation.name)
        if recommendation_name is None:
            recommendation_name = recommendation.name
        if recommendation.id == role_id or recommendation_name == role_name:
...
async def run_team(bot: Bot, ev: Event, char_name: str) -> None:
    ...
    matched = _filter_recommendations(recs, role_name, role_id)
    if not matched:
        return await send_nte_notify(bot, ev, TeamMsg.NO_RECOMMENDATION)

    await bot.send(await draw_team_img(matched, role_name))
